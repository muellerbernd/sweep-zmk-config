...
'inspired' by https://github.com/sporkus/zmk-config/blob/main/zmk_local_build.py
...
def zmk_build(kb: dict, git_branch: str) -> dict:
    ...
    build_status = {}
    cmd, target = get_build_cmd(
        board, shield, PATHS["zmk_config"], PATHS["output"], git_branch
    )
    if os.system(cmd) == 0:
...
def process_dirs(config: dict) -> None:
    for k, v in PATHS.items():
        PATHS[k] = os.path.join(FILE_DIR, os.path.expanduser(v))
...
def get_build_cmd(
    board: str, shield: str, config_dir: str, outdir: str, git_branch: str
) -> tuple:
    ...
    msg(
        f"Starting build script: shield={shield} board={board} \tOutput target: {output}"
    )
    return cmd, output
